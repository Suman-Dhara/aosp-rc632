...
#define	FB_DEV_PIXELCLOCK	(6000000)	/* 6Mhz */
...
{
	printk("nxp-fb.c: nxp_fb_dev_enable - 1.2\n");
#if defined CONFIG_NXP_DISPLAY //&& !defined(CONFIG_LOGO_NXP_COPY)
	int module = par->fb_dev.device_id;
	printk("nxp-fb.c: nxp_fb_dev_enable: module:%d - 1.3\n",module);
	int stat = 0;
	...
	if (!force){
		printk("nxp-fb.c: nxp_soc_disp_device_stat_enable: module:%d - 1.4\n",module);
		//stat = nxp_soc_disp_device_stat_enable(DISP_DEVICE_SYNCGEN0 + module);
		stat = nxp_soc_disp_device_enable(DISP_DEVICE_SYNCGEN0 + module,1);
	}
	if (!stat)
...
nxp_fb_init_display(struct fb_info *info)
{
	...
	printk("nxp-fb.c: nxp_fb_dev_enable - 1\n");
...
static int nxp_fb_setup_param(int fb, struct fb_info *info, void *data)
{
	...
	dev->layer    	  = plat->layer; pr_debug("dev->layer=%d\n",dev->layer);
	dev->x_resol_max  = plat->x_resol_max ? plat->x_resol_max : x_resol;pr_debug("dev->x_resol_max=%d\n",dev->x_resol_max);
	dev->y_resol_max  = plat->y_resol_max ? plat->y_resol_max : y_resol;pr_debug("dev->y_resol_max%d\n",dev->y_resol_max);
	dev->x_resol 	  = x_resol;pr_debug("dev->x_resol =%d\n",dev->x_resol );
	dev->y_resol 	  = y_resol;pr_debug("dev->y_resol=%d\n",dev->y_resol);
	dev->x_virt		  = x_resol;pr_debug("dev->x_virt=%d\n",dev->x_virt);
	dev->y_virt		  = PAGE_ALIGN(y_resol * plat->buffers);pr_debug("dev->y_virt=%d\n",dev->y_virt);
	dev->buffer_num	  = plat->buffers;pr_debug("dev->buffer_num=%d\n",dev->buffer_num);
	dev->pixelbit 	  = plat->bitperpixel;pr_debug("dev->pixelbit=%d\n",dev->pixelbit);
	dev->format		  = plat->format;pr_debug("dev->format=%d\n",dev->format);
	dev->bgcolor	  = plat->bgcolor;pr_debug("dev->bgcolor=%d\n",dev->bgcolor);
	dev->lcd_w_mm 	  = plat->lcd_with_mm;pr_debug("dev->lcd_w_mm=%d\n",dev->lcd_w_mm);
	dev->lcd_h_mm	  = plat->lcd_height_mm;pr_debug("dev->lcd_h_mm=%d\n",dev->lcd_h_mm);
	dev->fb_phy_base  = plat->fb_mem_base;pr_debug("dev->fb_phy_base=%d\n",dev->fb_phy_base);
	dev->fb_phy_end   = plat->fb_mem_end;pr_debug("dev->fb_phy_end=%d\n",dev->fb_phy_end);
	dev->fb_pan_phys  = plat->fb_mem_base;pr_debug("dev->fb_pan_phys=%d\n",dev->fb_pan_phys);

	dev->hs_left	  = vsi.h_sync_width + vsi.h_back_porch;pr_debug("dev->hs_left=%d\n",dev->hs_left);
	dev->hs_right	  = vsi.h_front_porch;pr_debug("dev->hs_right=%d\n",dev->hs_right);
	dev->vs_upper	  = vsi.v_sync_width + vsi.v_back_porch;pr_debug("dev->vs_upper=%d\n",dev->vs_upper);
	dev->vs_lower	  = vsi.v_front_porch;pr_debug("dev->layer=%d\n",dev->layer);
	dev->pixelclk	  = vsi.pixel_clock_hz ? vsi.pixel_clock_hz : FB_DEV_PIXELCLOCK;pr_debug("dev->pixelclk=%d\n",dev->pixelclk);
	dev->skip_vsync   = plat->skip_pan_vsync ? 1 : 0;pr_debug("dev->skip_vsync=%d\n",dev->skip_vsync);

	dev->fb_vir_base  = 0;
	dev->fb_remapped  = 0;

	pr_debug("%s (out dev=%d)\n", __func__, dev->pixelclk);
...
static void nxp_fb_setup_info(struct fb_info *info)
{
	...
	info->var.width	    	= dev->lcd_w_mm; 	/* width  mm for dpi */		pr_debug("info->var.width=%d\n",info->var.width);
	info->var.height	    = dev->lcd_h_mm; 	/* height  mm for dpi */	pr_debug("info->var.height=%d\n",info->var.height);
	info->var.bits_per_pixel= bpp;		pr_debug("info->var.bits_per_pixel=%d\n",info->var.bits_per_pixel);

	/* get frame rate */
	info->var.left_margin	= dev->hs_left;		pr_debug("info->var.left_margin=%d\n",info->var.left_margin);
	info->var.right_margin	= dev->hs_right;	pr_debug("info->var.right_margin=%d\n",info->var.right_margin);
	info->var.upper_margin	= dev->vs_upper;	pr_debug("info->var.upper_margin=%d\n",info->var.upper_margin);
	info->var.lower_margin	= dev->vs_lower;	pr_debug("info->var.lower_margin=%d\n",info->var.lower_margin);
	info->var.pixclock		= dev->pixelclk /*KHZ2PICOS(dev->pixelclk/1000)*/;	/* pico second */	pr_debug("info->var.pixclock=%d\n",info->var.pixclock);


	/* get resolution */
	info->var.xres	    	= dev->x_resol;		pr_debug("info->var.xres=%d\n",info->var.xres);
	info->var.yres	    	= dev->y_resol;		pr_debug("info->var.yres=%d\n",info->var.yres);
	info->var.bits_per_pixel= dev->pixelbit;	pr_debug("info->var.bits_per_pixel=%d\n",info->var.bits_per_pixel);
	info->var.xres_virtual 	= x_v;		pr_debug("info->var.xres_virtual=%d\n",info->var.xres_virtual);
	info->var.yres_virtual 	= y_v;		pr_debug("info->var.yres_virtual=%d\n",info->var.yres_virtual);

	/* get pixel format */
	info->var.red.offset  	= _G_BIT(bpp) + _B_BIT(bpp);		pr_debug("info->var.red.offset=%d\n",info->var.red.offset);
	info->var.green.offset  = _B_BIT(bpp);		pr_debug("info->var.green.offset=%d\n",info->var.green.offset);
	info->var.blue.offset   = 0;		pr_debug("info->var.blue.offset=%d\n",info->var.blue.offset);
	info->var.transp.offset = _T_BIT(bpp) ? (_R_BIT(bpp) + _G_BIT(bpp) + _B_BIT(bpp)) : 0;		pr_debug("info->var.transp.offset=%d\n",info->var.transp.offset);
	info->var.red.length    = _R_BIT(bpp);		pr_debug("info->var.red.length=%d\n",info->var.red.length);
	info->var.green.length  = _G_BIT(bpp);		pr_debug("info->var.green.length=%d\n",info->var.green.length);
	info->var.blue.length   = _B_BIT(bpp);		pr_debug("info->var.blue.length=%d\n",info->var.blue.length);
	info->var.transp.length = _T_BIT(bpp);		pr_debug("info->var.transp.length=%d\n",info->var.transp.length);

	/* other palette & fixed */
	info->pseudo_palette  = &par->pseudo_pal;		pr_debug("info->pseudo_palette=%d\n",info->pseudo_palette);
	info->fix.smem_len    = x_v * y_v * (bpp >> 3);		pr_debug("info->fix.smem_len=%d\n",info->fix.smem_len);
	info->fix.line_length = (info->var.xres * info->var.bits_per_pixel) >> 3;		pr_debug("info->fix.line_length=%d\n",info->fix.line_length);
	switch (bpp) {
		case 32:
		case 24:
		case 16:
